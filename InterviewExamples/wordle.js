...
// Write a fuc witj two paramaters
// loop thru guess and if it matches the postion in the answer array, we could have a result array which will have either G, _ Y
...
// (the guessed word could also be "spare" or some other fitting word)

// For now, don’t worry about runtime complexity.  

function guessAWord(priorGuess, priorResult, dictionary){
  // construct a string which we can use to get a list of possible matches from dictionary

  const stringsAtExactMatch = []
  const result = []

  let expectedStringGreenMap = new Map()
  let expectedStringYellowMap = new Map()
  for(let i = 0; i < priorResult.length; i++){
    if(priorResult[i] === 'G'){
      expectedStringGreenMap.set(priorGuess.slice(i, 1), i)
    }else if(priorResult[i] === 'Y'){
      expectedStringYellowMap.set(priorGuess.slice(i, i+1), i)
    }
    
  }

  for( let value of dictionary){
    for(let [charc, index] of expectedStringGreenMap){
      if(value.indexOf(charc) === index){
        stringsAtExactMatch.push(value)
      }
    }
  }

  for(let data of stringsAtExactMatch){
    for(let [charc] of Object.keys(expectedStringYellowMap){
      if(data.indexOf(charc) > 0){
        result.push(data)
      }
    }
  }
  ...
  console.log(result)
  return result.splice(pickRandomIndex(result.length), 1)[0]
...
console.log(guessAWord('solar', ["G", "_", "_", "Y", "Y"], dictionary))

// function buildGuessResult(answer, guess) {
//   // Having a map for answer
//   // have a result array/
//   // loop thru guess

//   const answerDictIndex = new Map()
//   const answerArr = answer.split('')
//   const result = []

//   for(let i= 0; i < answerArr.length; i++){
//     const charc = answerArr[i]
//     answerDictIndex.set(charc, (answerDictIndex.get(charc) || new Set()).add(i))
//   }
...
//     if(answerDictIndex.has(charc)){
//       const currentIndex = answerDictIndex.get(charc)

//       if(currentIndex.has(i)){ // TODO need to check for duplicate charc
//         result.push('G')  
//       } else {
//         result.push('Y')  
//       }
...
// console.log(buildGuessResult('spare', 'solar')) // ["G", "_", "_", "Y", "Y"]
// console.log(buildGuessResult('piano', 'logic')) // ["_", "Y", "_", "Y", "_"]
// console.log(buildGuessResult('answe', 'trttt')) // all needs to be underscore
// console.log(buildGuessResult('answe', '')) // all needs to be underscore

// console.log(buildGuessResult('tnswa', 'answa')) // [_, G, G, G, G]